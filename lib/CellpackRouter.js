...
class CellpackRouter extends microb_1.Cellpack {
    ...
    path(routeName, params) {
        if (this.environment.get("debug"))
            this.transmitter.emit("log.cellpack.router", `Render path for ${routeName} with ${params}`);
        let error = "";
        params = params || {};
    ...
    url(uri, params) {
        if (this.environment.get("debug"))
            this.transmitter.emit("log.cellpack.router", `Render path for ${uri} with ${params}`);
        let error = "";
        params = params || {};
